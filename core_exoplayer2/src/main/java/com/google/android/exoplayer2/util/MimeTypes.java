...
public final class MimeTypes {
  ...
  private static final ArrayList<MimeTypes.CustomMimeType> customMimeTypes = new ArrayList();

  public static void registerCustomMimeType(String mimeType, String codecPrefix, int trackType) {
    MimeTypes.CustomMimeType customMimeType = new MimeTypes.CustomMimeType(mimeType, codecPrefix, trackType);
    int customMimeTypeCount = customMimeTypes.size();

    for(int i = 0; i < customMimeTypeCount; ++i) {
      if (mimeType.equals(((MimeTypes.CustomMimeType)customMimeTypes.get(i)).mimeType)) {
  ...
  private static String getCustomMimeTypeForCodec(String codec) {
    ...
    for(int i = 0; i < customMimeTypeCount; ++i) {
      MimeTypes.CustomMimeType customMimeType = (MimeTypes.CustomMimeType)customMimeTypes.get(i);

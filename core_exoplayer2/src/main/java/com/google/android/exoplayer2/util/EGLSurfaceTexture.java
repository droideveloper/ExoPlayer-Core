...
public final class EGLSurfaceTexture implements OnFrameAvailableListener, Runnable {
  ...
  public EGLSurfaceTexture(Handler handler) {
    this(handler, (EGLSurfaceTexture.TextureImageListener)null);
  }
  ...
  private static EGLConfig chooseEGLConfig(EGLDisplay display) {
    ...
    if (success && numConfigs[0] > 0 && configs[0] != null) {
      ...
      throw new EGLSurfaceTexture.GlException(Util.formatInvariant("eglChooseConfig failed: success=%b, numConfigs[0]=%d, configs[0]=%s", new Object[]{success, numConfigs[0], configs[0]}));
    }
  }
  ...
  private static void generateTextureIds(int[] textureIdHolder) {
    ...
    if (errorCode != 0) {
      throw new EGLSurfaceTexture.GlException("glGenTextures failed. Error: " + Integer.toHexString(errorCode));
    }
  }
